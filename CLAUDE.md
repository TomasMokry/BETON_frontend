...
- `src/services/fetchWithAuth.ts` is a fetch wrapper used for all authenticated API calls: it checks JWT expiry client-side before each request, refreshes proactively if needed, retries once on a `401`, and hard-redirects to `/` (via `window.location.href`, not React Router) if refresh fails. Note: this file currently hardcodes `http://localhost:8080/auth/refresh` instead of using `BASE_URL` from `config.ts` — be aware of this inconsistency with `AuthContext.tsx` (which does use `BASE_URL`) when touching auth/refresh logic.
